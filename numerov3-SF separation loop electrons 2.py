...
DeltaSF = 0.7775*(10**(-9))   
Eoff = 0.27                         #band offset between Hex/Cubic GaN
#Epot = 0.303                #included calculation for this parameter
hbar = 6.582119514E-16
me = 510998.9
m=0.2*me                              #effective mass of carrier 
#Steps = 1000
MaxSwitch = 20
...
L=lab.linspace(5*DeltaSF,400*DeltaSF/3,No_sep)      #separation of SFs to solve equation
ground_states = [0]*No_sep                  #array to store ground state energies
SF_drops = [0]*No_sep
s=0                                         #SF separation loop tracking parameter
No_periods = 1     #in potential
PSIout = []
...
while s<No_sep:
    ...
    MaxZ = (No_periods*DeltaX+(No_periods+1)*DeltaSF)/DeltaX   #dimensionless potential range
    Epot = -Phex/(eps*((ephex/DeltaSF)+(epcub/(2*DeltaX))))      #output in eV
    stepsize = 0.01E-9
    Steps = (No_periods*DeltaX+(No_periods+1)*DeltaSF)/stepsize
    StepSize =MaxZ/Steps
    ...
    def V(z,No_periods):
        ...
        if x <DeltaSF:                               #inside SF
            return ((-Epot)/(DeltaSF))*x+(Epot+Eoff)
        for n in range(1,No_periods+1):
            if ((n-1)*DeltaX+(n)*DeltaSF)<x<(n*DeltaX+n*DeltaSF):                      #outside SF
                return (Epot/DeltaX)*(x-n*DeltaSF-(n-1)*DeltaX)
            elif (n*DeltaX+n*DeltaSF)<x<n*DeltaX+(n+1)*DeltaSF:                      #outside SF
                return ((-Epot)/(DeltaSF))*(x-n*DeltaX-n*DeltaSF)+(Epot+Eoff)
        return 0
    ...
    potential = []                                  
    for Val in xRange:
        potential.append(V(Val,No_periods)/E_0)
...
for i in range(0,(No_sep)):
    file.write(str(L[i])+","+str(SF_drops[i])+","+str(ground_states[i])+"\n")
file.close()

file=open("wf.txt", 'w')
for i in range(len(y_out)):
	file.write(str(x_out[i])+','+str(y_out[i]) +"\n")
